...
class TonWalletProvider {
    ...
    async createWalletFromMnemonic(mnemonic, path) {
        try {
            console.log('[createWallet:ton] mnemonic ');
            let keyPair = await (0, crypto_1.mnemonicToPrivateKey)(mnemonic.split(' '));
            const workchain = 0;
            ...
            console.log("[createWallet:ton] error ", e.message);
            return {
                success: false,
                error: e.message,
            };
    ...
    async createWallet() {
        try {
            console.log('[createWallet:ton] no private key or mnemonic, creating a new wallet');
